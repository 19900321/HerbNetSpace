...
def parse_args():
    ...
    parser.add_argument('--transmatrixf',type=str,default='trans_matrix.pkl',
                       help='trans_matrix file between different type of node; default = file path')
    parser.add_argument('--walk_pathf',type=str,default='tmp_walk_file.pkl',
                       help='walk path file, default = file path')
    ...
    args={
        ...
        'transmatrixf':args.transmatrixf,
        'walk_pathf':args.walk_pathf,
        'seed':args.seed,
...
#     print(f'# of walkers : {min(num_walks,workers)} for {num_walks} times')
    ...
    walks = [_parmap_walks(nodes, G, G_sim, trans_matrix, p, q, walk_length, tp_factor)
...
# parallel walks
def _parmap_walks(nodes,G,G_sim,trans_matrix,p,q,walk_length,tp_factor):
    walks = []
...
def save_embedding_files(netf:str,
                         sim_netf:str,
                         outputf:str,
                         nodetypef:str=None,
                         transmatrixf:str=None,
                         walk_pathf:str=None,
                         tp_factor:float=0.7, seed:int=42,
                         directed:bool=False, weighted:bool=True, em_max_iter:int=5,
                         num_walks:int=50, walk_length:int=5, workers:int=os.cpu_count(), 
    ...
    # generate translation matrix and embedding 
    ...
    with open(transmatrixf,'wb') as fw_trans:
        pickle.dump(trans_matrix,fw_trans)
    
    walks=generate_DREAMwalk_paths(G,G_sim,trans_matrix,p,q,num_walks,
                                   walk_length,tp_factor, workers)
    
    with open(walk_pathf,'wb') as fw:
        pickle.dump(walks,fw)
    
    #walks = pickle.load(open(walk_pathf, 'rb'))
